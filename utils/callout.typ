...
#let styles = (
  ...
  "convention": (
    ...
    "neg-color": rgb("#FFFFFF"),
    "text-color": rgb("#000000"),
    "icon": octique-inline("repo", color: rgb("#ffffff"), width: 1em, height: 1em, baseline: 25%),
    "prefix": "Convention"
  ),
    "assumption": (
    "color": rgb("#59857ae8"),
...
#let callout(title: "", style: "standard", ignore-prefix: false, label_: none, content) = {
  ...
  // Tracks, per callout style, whether the title bar currently being laid
  // out is the first one (top of the box) or a repeat that was pushed onto
  // a following page because the content didn't fit.
  let continued-state = state("callout-continued-" + style)

  show figure.caption: none
  // Allow the callout's box to break across a page boundary instead of
  // being pushed as one atomic unit (which is what caused overflowing
  // content to get silently clipped).
  ...
  [
    // Reset the continuation tracker right before this callout's table so
    // multiple callouts on the page don't interfere with each other.
    #show table: it => {
      continued-state.update(false)
      it
    }
    #block(
      ...
      table(
        ...
        // table.header(repeat: true) re-lays-out this cell at the top of
        // every page the table spans, which is what lets us detect (via
        // the state above) whether we're drawing the original title bar
        // or one that continues onto a new page.
        table.header(
          repeat: true,
          table.cell(
            ...
            #context {
              let is-continued = continued-state.get()
              continued-state.update(true)
              let suffix = if is-continued { " (continued)" } else { "" }
              ...
              let text-part = if "prefix" in style_dict and not ignore-prefix {
                ...
                if title != "" {
                  text(fill: title-color, weight: "bold")[#prefix-text: #title#suffix]
                } else {
                  text(fill: title-color, weight: "bold")[#prefix-text#suffix]
                }
              } else {
                text(fill: title-color, weight: "bold", title + suffix)
              }
...
#callout(title: "Callout being broken up over multiple pages")[
  #lorem(1000)
]

#callout(title: "numbers")[
  1
2
3
4
